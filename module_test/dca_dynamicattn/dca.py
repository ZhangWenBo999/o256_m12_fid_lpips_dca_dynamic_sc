...
# 目前的CSAM模块将语义、位置和切片注意力模块串联处理，这种固定的处理顺序可能在不同的任务或不同数据分布中表现不佳。
# 为了提高模块的灵活性和适应性，动态注意力融合机制允许网络根据输入数据自适应调整注意力模块的权重，从而动态聚合语义、位置和切片注意力的信息。
class AttentionFusionModule(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        # 动态注意力融合机制通过一个权重生成模块，根据输入特征计算每中注意力模块的权重，并将模块输出进行加权融合。
        weights = self.softmax(self.weights)  # 计算动态权重


        # for w, inp1, inp2 in zip(weights, inputs1, inputs2):
        #     test = w * (inp1 + inp2)
        #     print(test)

        fused_output = [w * (inp1 + inp2) for w, inp1, inp2 in zip(weights, inputs1, inputs2)] # 加权融合
        return fused_output
...
class CCSABlock(nn.Module):
    def __init__(self, 
        ...
        if self.spatial_att:
            ...
            self.AttentionFusionModule = AttentionFusionModule(12)

    def forward(self, x):
        if self.channel_att:
            x_ca = self.channel_attention(x)
            x_ca = self.m_sum(x, x_ca)
        if self.spatial_att:
            x_sa = self.spatial_attention(x)
            x_sa = self.m_sum(x, x_sa)
        return self.AttentionFusionModule(x_ca, x_sa)
